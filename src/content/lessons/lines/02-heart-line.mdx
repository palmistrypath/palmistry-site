...
<Figure src="/images/lessons/lines/linesl204.webp" alt="Illustration: a single palm with three overlaid versions of the heart line's endpoint, each ending in a different position. The first ends directly below the index finger (labeled 'Jupiter'). The second ends in the gap between the index and middle fingers (labeled 'Jupiter–Saturn'). The third ends below the middle finger (labeled 'Saturn'). Each endpoint is marked with a distinct symbol. The finger bases are labeled at the top of the palm. Other lines are shown faintly." caption="The three main endpoints of the heart line. The endpoint below Saturn is where Western and Indian traditions diverge most significantly." />
...
<Figure src="/images/lessons/lines/linesl205.webp" alt="Illustration: three close-up versions of a heart line endpoint. The first shows a clean single termination. The second shows the line forking into two distinct branches as it ends. The third shows a fine upward branch rising from the line toward a finger mount. Each version is labeled: 'clean,' 'forked,' 'upward branch.' Other lines are faint for spatial reference." caption="Endpoint variations: clean termination, a fork, and an upward branch." />
